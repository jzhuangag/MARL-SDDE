...
3. Minimizing this bound by exact discrete `(q,b)` enumeration and certified scalar step search gives a low-complexity mixed controller rather than a heuristic participation rule.
...
7. Three registered mechanism studies and one seed-separated convergence confirmation validate correlation saturation, joint action response, finite-time coverage, safety, learning-value separation, and return-estimation curves.
...
| III. Model and Objective | Define fixed-policy evaluation, data, costs, delay, and finite-time risks | 12% |
| IV. Correlation and Resource Geometry | Establish exact spatial and temporal phase structure | 10% |
| V. Lyapunov-Certified Resource Control | Derive the affine bound and joint `(q,b,eta)` selection | 17% |
| VI. Learning-Aware Dependence Adaptation | Derive adaptive information lower bound, safe upper rule, and threshold theorem | 17% |
| VII. Implementation and SDDE Interpretation | Give the end-to-end algorithm, confidence interface, complexity, and design interpretation | 10% |
| VIII. Experiments | Test mechanism, certificate, learning-value, and convergence predictions | 18% |
| IX. Conclusion | State the solved problem and significance | 4% |
...
| Joint control improves learning trajectories | Return-transfer corollary and finite-budget Lyapunov score | TSP-CURVE-001 held-out parameter and return-estimation curves |
