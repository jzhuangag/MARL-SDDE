...
3. Create one Slurm job per cell of the frozen 16-cell grid: coupling in
   ...
   The scientific command template is:
...
6. Aggregate deterministic team return on charged budget fraction.  Select the
   global strong fixed `(q, eta)` over both coupling regimes and the best fixed
   pair separately in each regime.  Report the per-regime oracle and its
   aggregate return-AUC headroom over the global strong fixed pair.  Do not use
   a smoothed curve for numerical gates; smoothing is visualization-only.
