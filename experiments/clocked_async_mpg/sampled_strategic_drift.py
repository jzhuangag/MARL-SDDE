"""Sample-split strategic-drift learner with fully charged Markov packets."""
...
def simulate_sample_split_strategic_drift(
    *,
    coupling: float,
    service_ratio: float,
    seed_index: int,
    ...
    """Use independent half-packets for proposal and directional validation.

    The two halves jointly contain exactly ``batch_size`` trajectories, so the
    controller is charged the same Markov transition work as each comparator.
    No confidence radius is subtracted here: this is an unbiased predictable
    directional-value interface, not a high-probability no-harm certificate.
    """
    ...
    def packet(agent: int) -> tuple[np.ndarray, np.ndarray]:
        proposal = sample_reinforce_packet(
            logits, game, agent, horizon, half, trajectory_rng[agent]
        )
        validation = sample_reinforce_packet(
            logits, game, agent, horizon, half, trajectory_rng[agent]
        )
        return proposal, validation

    # completion, agent, birth version, duration, proposal, validation, birth
    queue: list[
        tuple[float, int, int, float, np.ndarray, np.ndarray, np.ndarray]
    ] = []
    for agent in range(2):
        duration = _service_duration(service_rng[agent], agent, service_ratio)
        proposal, validation = packet(agent)
        heapq.heappush(
            queue,
            (
                duration,
    ...
    while queue:
        ...
        direction = base_step*proposal
        directional_gain = float(validation@direction)
        curvature_penalty = (
            0.5
        ...
        next_proposal, next_validation = packet(agent)
        heapq.heappush(
            queue,
            (
    ...
    return {
        ...
        "mean_estimated_lower_bound": float(np.mean(lower_bounds)),
