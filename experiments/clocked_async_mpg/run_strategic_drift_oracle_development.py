"""Development-only oracle-headroom comparison for strategic drift scaling."""
...
def run(*, seeds: int, workers: int, namespace: str) -> dict[str, Any]:
    ...
    for coupling in couplings:
        for ratio in ratios:
            ...
            candidate_rows = [
                indexed[(coupling, ratio, seed, "oracle_debt")]
                for seed in range(seeds)
            ]
            cell["oracle_debt_mean_scale"] = statistics.mean(
                float(row["mean_scale"]) for row in candidate_rows
            )
            cell["oracle_debt_mean_terminal_debt"] = statistics.mean(
            ...
            for comparator in POLICIES[1:]:
                ...
                for seed in range(seeds):
                    candidate = indexed[(coupling, ratio, seed, "oracle_debt")]
                ...
                cell[f"vs_{comparator}_time_ratio"] = (
                    statistics.median(time_ratios) if time_ratios else None
                )
                cell[f"vs_{comparator}_work_ratio"] = (
    ...
    for population, selected in (
        ...
        for comparator in POLICIES[1:]:
            times = [
                float(cell[f"vs_{comparator}_time_ratio"])
                for cell in selected
                if cell[f"vs_{comparator}_time_ratio"] is not None
            ]
            works = [
                float(cell[f"vs_{comparator}_work_ratio"])
                for cell in selected
                if cell[f"vs_{comparator}_work_ratio"] is not None
            ]
            ...
            aggregate[population][comparator] = {
                ...
                "coverage": len(times)/len(selected),
    ...
    return {
        ...
        "fixed_oracle_debt_parameters": {"risk_budget": 0.001, "tradeoff": 10.0},
        "job_count": len(jobs),
        "namespace": namespace,
...
def main() -> None:
    ...
    parser.add_argument(
        "--namespace", default="strategic-drift-oracle-development-v1"
    )
