...
def test_score_scale_audit_reconstructs_index_and_ignores_return(tmp_path: Path) -> None:
    ...
    assert result["scale_audit_passed"]
...
def test_score_scale_audit_fails_closed_for_zero_learning_component(
    tmp_path: Path,
) -> None:
    record = {
        "scheduler": "signed_lyapunov",
    ...
    source.write_text(json.dumps(record), encoding="utf-8")
    result = analyze(source)
    assert not result["learning_component_nonzero"]
    assert result["recommended_learning_weight"] is None
    assert not result["scale_audit_passed"]
