"""Run EXP-006B observable state-and-correlation participation control."""
...
def bootstrap_ratio(
    ...
    values = np.mean(numerator[indices], axis=1) / np.mean(
        denominator[indices], axis=1
    )
...
def build_comparisons(
    ...
    adaptive = seed_scores(metrics, "state_correlation_adaptive")
    comparisons = [
        (
            "state_vs_correlation_only",
            "correlation_only_adaptive",
            base_seed + 10000,
        ),
        ("state_vs_best_fixed", best_fixed, base_seed + 10001),
        ("state_vs_oracle", "state_oracle", base_seed + 10002),
    ]
    rows = []
    for name, denominator, seed in comparisons:
        rows.append(
            {
                "comparison": name,
                "numerator": "state_correlation_adaptive",
...
def action_agreement(actions: pd.DataFrame) -> Dict[str, float]:
    ...
    adaptive = actions[
        actions["policy"] == "state_correlation_adaptive"
...
def evaluate_gates(
    ...
    state_value = lookup.loc["state_vs_correlation_only"]
    fixed = lookup.loc["state_vs_best_fixed"]
    oracle = lookup.loc["state_vs_oracle"]
    ...
    expected_runs = 64 * len(SCENARIOS) * len(MAX_DELAYS) * len(POLICIES)
    validity = bool(
    ...
    gates["overall"] = {
        ...
        "criterion": "all six observable-controller gates pass",
    }
    return gates
...
def plot_outputs(
    ...
    fig.suptitle("Observable state-and-correlation controller")
    fig.tight_layout()
    ...
    selected = actions[
        actions["policy"].isin(
            ["state_correlation_adaptive", "state_oracle"]
        )
    ]
    summary = (
    ...
    for ax, scenario in zip(axes.ravel(), SCENARIOS):
        for policy in ("state_correlation_adaptive", "state_oracle"):
            subset = summary[
    ...
    ax.scatter(
        ...
        adaptive["observable_state_proxy"],
        s=8,
        alpha=0.25,
    )
    limits = [1e-3, 1.0]
    ...
    ax.set_ylabel("Observable gradient state proxy")
    ax.set_title("State-proxy calibration")
    fig.tight_layout()
    fig.savefig(output_dir / "fig3_state_proxy.png")
    plt.close(fig)
...
def main() -> None:
    ...
    config = StateCorrelationConfig()
    print(
        "Starting EXP-006B with {0} paired seeds".format(args.num_seeds),
        flush=True,
    )
    errors, actions, runs = run_experiment(
        config, args.num_seeds, args.base_seed
    )
    metrics = build_cell_metrics(errors, config)
    ...
    gates = evaluate_gates(
        comparisons, actions, runs, metrics, config
    )
    metrics.to_csv(
    ...
    summary = {
        "experiment_id": "EXP-006B-state-correlation-controller",
